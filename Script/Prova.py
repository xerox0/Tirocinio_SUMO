...
for person in root.findall('.//person'):
    ...
    timestamp = str(person.get('timestep'))
    ...
    if person_id not in [p[0] for p in grid[cell]]:
        grid[cell].append((person_id, x, y, timestamp))
...
def get_cell_stats(grid):
    ...
    for cell, people in grid.items():
        ...
        timestamps = [p[3] for p in people]
        cell_stats[cell] = {
        ...
        for person in people:
            person_id = person[0]
            timestamp = person[3]
            if timestamp not in cell_stats[cell]['visits']:
                cell_stats[cell]['visits'][timestamp] = [person_id]
            else:
                cell_stats[cell]['visits'][timestamp].append(person_id)
    return cell_stats
...
# Aggiungiamo una colonna 'timestamp' per ogni visita
for i in range(len(results)):
    visits = results.iloc[i]['visits']
    timestamps = sorted(visits.keys())
    for timestamp in timestamps:
        results.at[i, f'timestamp_{timestamp}'] = ', '.join(visits[timestamp])

# Eliminiamo la colonna 'visits' e salviamo l'output in un file CSV
results.drop(['visits'], axis=1, inplace=True)
results.to_csv('/home/toore/cell_stats_prova.csv', index=False)